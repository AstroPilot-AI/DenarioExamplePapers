...
Below we show a list of papers generated by Denario across scientific disciplines. We note that we have not tried to filter the papers. For that reason, beyond the papers shown in the Denario presentation paper, we also include papers that can be good or bad. We do this to illustrate that Denario can generate both innovative papers but also bad ones, so human expertise is currently needed to evaluate the paper.

We note that the papers below were generated across several months while developing and improving the software. For that reason, the quality, quantity, style, and even author may change from one paper to another. On top of that, some modules have been added later than others, so there may be papers with some folders, containing the output of a module, missing. 
...
To download the material associated with a paper you can use, e.g.,:

```
wget -r -np -e robots=off --reject “index.html*” https://users.flatironinstitute.org/~fvillaescusa/Denario/Quantum+Cosmology/Project1/
```


## Contributing

If you generate an interesting article with [Denario](https://github.com/AstroPilot-AI/Denario) please feel free to open a pull request with the full project files (including code and latex files).
