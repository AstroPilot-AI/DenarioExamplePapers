# filename: codebase/gcn_model_training.py
import torch
...
def train_gnn_model(model, train_loader, val_loader, optimizer, criterion, num_epochs, model_save_dir):
    ...
    for epoch in range(num_epochs):
        ...
        for batch_data in train_loader:
            ...
            # Ensure target is [batch_size, 1] and same dtype as output
            target = batch_data.y.unsqueeze(1).to(out.dtype) 
            loss = criterion(out, target)
            
            if torch.isnan(loss):
                print("Warning: NaN loss encountered in training at epoch " + str(epoch + 1) + ". Skipping batch.")
                # Potentially skip this batch or stop training if NaNs persist
                # For now, we'll record it as NaN and continue, but this indicates instability
                total_train_loss += float('nan')  # Propagate NaN
            else:
                loss.backward()
        ...
        with torch.no_grad():
            for batch_data in val_loader:
                ...
                loss = criterion(out, target)
                if torch.isnan(loss):
                    print("Warning: NaN loss encountered in validation at epoch " + str(epoch + 1) + ".")
                    total_val_loss += float('nan')
                else:
        ...
        train_loss_str = "{:.4f}".format(avg_train_loss) if not np.isnan(avg_train_loss) else "nan"
        val_loss_str = "{:.4f}".format(avg_val_loss) if not np.isnan(avg_val_loss) else "nan"
        print("Epoch " + str(epoch + 1) + "/" + str(num_epochs) + ": Train Loss: " + train_loss_str + ", Val Loss: " + val_loss_str)
    ...
    best_val_loss_str = "{:.4f}".format(best_val_loss) if not np.isinf(best_val_loss) else "inf"
    print("Best validation loss achieved: " + best_val_loss_str)
...
if __name__ == '__main__':
    ...
    if train_dataset:
        ...
        if hasattr(first_data, 'edge_attr') and first_data.edge_attr is not None and first_data.edge_attr.numel() > 0:
            ...
            elif first_data.edge_attr.dim() == 1:  # if 1D tensor of features per edge
                num_edge_features = 1 
    ...
    for n_layers in param_grid['num_gcn_layers']:
        for h_channels in param_grid['hidden_channels']:
            ...
            final_val_loss_for_config = val_losses[-1] if val_losses and not np.isnan(val_losses[-1]) else float('inf')
            
            final_val_loss_str = "{:.4f}".format(final_val_loss_for_config) if not np.isinf(final_val_loss_for_config) else "inf"  # or "nan" if it can be nan
            if np.isnan(final_val_loss_for_config): final_val_loss_str = "nan"

            print("Final validation loss for " + current_hyperparams_str + ": " + final_val_loss_str)
            results_log.append({
                'layers': n_layers, 'hidden': h_channels, 
                'final_val_loss': final_val_loss_for_config,  # Store the actual float value
                'train_losses': train_losses, 'val_losses': val_losses
            })

            if not np.isnan(final_val_loss_for_config) and not np.isinf(final_val_loss_for_config):
    ...
    if best_hyperparams:
        ...
        overall_best_val_loss_str = "{:.4f}".format(overall_best_val_loss) if not np.isinf(overall_best_val_loss) else "inf"
        print("  Corresponding best validation loss: " + overall_best_val_loss_str)
    else:
    ...
    print("\nTotal time for GNN design, training setup, and hyperparameter tuning: " + "{:.2f}".format(end_time_main - start_time_main) + " seconds.")
