# filename: codebase/feature_engineering.py
import torch
import numpy as np
...
def extract_features_for_graph(graph_data, scaler_x_fitted):
    ...
    try:
        # Ensure x_original is 2D, even if num_nodes is 1
        if x_original.ndim == 1:
            x_original = x_original.reshape(1, -1)
        if x_original.shape[0] == 0 : # No nodes
    ...
    if not isinstance(laplacian_eigenvalues_all, np.ndarray) or laplacian_eigenvalues_all.size == 0:
        ...
        # Check for std_eig being zero or very small before calculating skew/kurtosis
    ...
    if num_nodes >= 2: 
        try:
            ...
            if k_diff_eig > 0:
                ...
                mean_pooled_diff = np.nanmean(node_embeddings_diffusion, axis=0) # Use nanmean
                max_pooled_diff = np.nanmax(node_embeddings_diffusion, axis=0)   # Use nanmax
                min_pooled_diff = np.nanmin(node_embeddings_diffusion, axis=0)   # Use nanmin
...
if __name__ == '__main__':
    ...
    all_lh_ids = np.array([data.lh_id for data in full_dataset]) # Corrected: .item() removed
    ...
    for i in train_indices:
        # Check if graph has 'x' and it's not empty
    ...
    if not all_train_x_list:
        ...
        normalization_params = {'mean': np.zeros(full_dataset[0].x.shape[1] if len(full_dataset) > 0 and hasattr(full_dataset[0], 'x') else 4), 
                                'std': np.ones(full_dataset[0].x.shape[1] if len(full_dataset) > 0 and hasattr(full_dataset[0], 'x') else 4)} 
    else:
    ...
    for i, graph_idx in enumerate(train_indices):
        ...
        processed_train_data['lh_ids'].append(original_graph.lh_id) # Corrected
        if (i + 1) % 100 == 0:
    ...
    for i, graph_idx in enumerate(test_indices):
        ...
        processed_test_data['lh_ids'].append(original_graph.lh_id) # Corrected
        if (i + 1) % 50 == 0:
    ...
    if processed_train_data['X'].size > 0 : # Check if array is not empty
    ...
    if processed_test_data['X'].size > 0: # Check if array is not empty
