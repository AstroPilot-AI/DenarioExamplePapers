# filename: codebase/dimensionality_reduction_baseline_features.py
import numpy as np
import torch
...
all_lh_ids_original = np.array([data.lh_id.item() for data in full_dataset_original])  # Use .item() for tensor lh_id
...
# Based on Step 1 output, X_train_engineered.shape[0] == len(original_train_indices)
# and X_test_engineered.shape[0] == len(original_test_indices),
# implying no graphs were entirely skipped during feature engineering in Step 1.
# If this assumption changes, this part needs adjustment.
...
if node_feature_std is not None and len(node_feature_std) > 0:  # Avoid error if std is None or empty
...
def get_baseline_features_for_graph(graph_data, scaler):
    ...
    num_expected_node_features = scaler.n_features_in_ if hasattr(scaler, 'n_features_in_') else 4  # Default to 4 if not set
    ...
    if x_original.shape[1] != num_expected_node_features:
        # print("Warning: Graph node features " + str(x_original.shape[1]) + " != scaler expected " + str(num_expected_node_features))
        return np.full(num_expected_node_features * 4, np.nan)

    try:
        # Ensure scaler is properly initialized for transform
        if not hasattr(scaler, 'mean_') or not hasattr(scaler, 'scale_') or scaler.mean_ is None or scaler.scale_ is None:
            # print("Warning: Scaler not properly fitted/initialized for baseline features.")
            return np.full(num_expected_node_features * 4, np.nan)
        x_norm = scaler.transform(x_original)
    except Exception as e:
        # print("Warning: Could not normalize x for baseline. Graph num_nodes: " + str(graph_data.num_nodes) + ". Error: " + str(e))
        return np.full(num_expected_node_features * 4, np.nan)

    # Check if x_norm is empty after potential issues
    if x_norm.shape[0] == 0:
        ...
        with np.warnings.catch_warnings():  # Suppress warnings from nanmin/max on all-NaN slices
            np.warnings.filterwarnings('ignore', r'All-NaN (slice|axis) encountered')
            np.warnings.filterwarnings('ignore', r'Mean of empty slice')
            np.warnings.filterwarnings('ignore', r'Degrees of freedom <= 0 for slice')  # for std of single element
            means = np.nanmean(x_norm, axis=0)
            stds = np.nanstd(x_norm, axis=0)
            mins = np.nanmin(x_norm, axis=0)
            maxs = np.nanmax(x_norm, axis=0)
            
            # Ensure stds for single-node graphs are 0, not NaN
            if x_norm.shape[0] == 1:
...
if num_nans_baseline_train > 0:
    ...
    print("  Indices of rows with NaNs in X_train_baseline: " + str(nan_rows_train_baseline[:5]))  # Print first 5
...
if num_nans_baseline_test > 0:
    ...
    print("  Indices of rows with NaNs in X_test_baseline: " + str(nan_rows_test_baseline[:5]))  # Print first 5
