# filename: codebase/data_preprocessing_feature_engineering.py
import torch
import numpy as np
...
MAX_NODES_FOR_EIGS = 500 # Skip eigs for graphs larger than this to prevent timeout
...
def extract_features_for_graph(graph_data, scaler_x_fitted):
    ...
    # Define the total number of features for consistent NaN array creation
    num_edge_features = 4
    num_laplacian_features = 5  # 4 moments + 1 sum
    num_diffusion_features = NUM_DIFFUSION_EIGENVECTORS * 3
    num_total_features = num_edge_features + num_laplacian_features + num_diffusion_features

    if not hasattr(graph_data, 'x') or graph_data.x is None:
        # print("Warning: Graph data has no 'x' attribute.")
    ...
    try:
        ...
        # print("Warning: Could not normalize x for graph with " + str(num_nodes) + " nodes. Error: " + str(e))
        return np.full(num_total_features, np.nan)

    # 1. Edge-Level Features
    if edge_index.shape[1] > 0 and x_norm.shape[0] > 0:  # Ensure nodes exist for indexing
        sources, targets = edge_index[0].numpy(), edge_index[1].numpy()
        # Check if sources/targets are within bounds of x_norm
        if np.any(sources >= x_norm.shape[0]) or np.any(targets >= x_norm.shape[0]):
            # print("Warning: Edge indices out of bounds for node features in graph with " + str(num_nodes) + " nodes.")
            ...
            delta_sf_list = np.abs(sf_diffs)
            log_mass_ratio_list = mass_ratio_logs
    ...
    laplacian_features = np.full(num_laplacian_features, np.nan)
    if num_nodes > MAX_NODES_FOR_EIGS:
        # print("Skipping Laplacian eigs for large graph with " + str(num_nodes) + " nodes.")
        pass  # laplacian_features remains NaNs
    elif num_nodes > 0:
        try:
            ...
            if not isinstance(laplacian_eigenvalues_all, np.ndarray) or laplacian_eigenvalues_all.size == 0:
                ...
                skew_eig = skew(laplacian_eigenvalues_all) if not np.isclose(std_eig, 0) else 0.0
                kurt_eig = kurtosis(laplacian_eigenvalues_all, fisher=True) if not np.isclose(std_eig, 0) else 0.0
                ...
                sum_smallest_nonzero_eig = np.sum(non_zero_eigs_sorted[:NUM_SMALLEST_EIG_SUM]) if len(non_zero_eigs_sorted) > 0 else 0.0
            laplacian_features = np.concatenate([spectral_moments, np.array([sum_smallest_nonzero_eig])])

        except Exception as e:
            # print("Warning: Laplacian computation failed for graph with " + str(num_nodes) + " nodes. Error: " + str(e))
            # laplacian_features remains NaNs
            pass
    
    # 3. Diffusion Map Features
    diffusion_aggregated_features = np.full(num_diffusion_features, np.nan)
    if num_nodes > MAX_NODES_FOR_EIGS:
        # print("Skipping Diffusion Map eigs for large graph with " + str(num_nodes) + " nodes.")
        pass  # diffusion_aggregated_features remains NaNs
    elif num_nodes >= 2:
        try:
            ...
            if k_diff_eig > 0:
                eigvals_P, eigvecs_P = eigs(P, k=k_diff_eig, which='LR', tol=1e-4, maxiter=100)  # Added tol and maxiter
                ...
                if eigvecs_P_sorted_real.shape[1] > 0:  # Check if any eigenvectors were found
                    node_embeddings_diffusion[: , :eigvecs_P_sorted_real.shape[1]] = eigvecs_P_sorted_real

                mean_pooled_diff = np.nanmean(node_embeddings_diffusion, axis=0)
                max_pooled_diff = np.nanmax(node_embeddings_diffusion, axis=0)
                min_pooled_diff = np.nanmin(node_embeddings_diffusion, axis=0)
                diffusion_aggregated_features = np.concatenate([mean_pooled_diff, max_pooled_diff, min_pooled_diff])
        except Exception as e:  # Catch specific sparse eigs errors if possible
            # print("Warning: Diffusion map computation failed for graph with " + str(num_nodes) + " nodes. Error: " + str(e))
            pass 
...
if __name__ == '__main__':
    ...
    all_lh_ids = np.array([data.lh_id for data in full_dataset])
    ...
    train_indices = [i for i, lh_id_val in enumerate(all_lh_ids) if lh_id_val in train_simulation_ids]
    test_indices = [i for i, lh_id_val in enumerate(all_lh_ids) if lh_id_val in test_simulation_ids]
    ...
    if len(train_indices) > 0:
        # Attempt to get num_node_features from the first valid graph
        for i in train_indices:
            current_graph_x_temp = getattr(full_dataset[i], 'x', None)
            if current_graph_x_temp is not None and current_graph_x_temp.shape[0] > 0:
                num_node_features = current_graph_x_temp.shape[1]
                break
        if num_node_features == 0 and len(full_dataset) > 0:  # Fallback if no valid train graph, try any graph
             current_graph_x_temp = getattr(full_dataset[0], 'x', None)
             if current_graph_x_temp is not None:
                 num_node_features = current_graph_x_temp.shape[1]

    for i in train_indices:
        current_graph_x = getattr(full_dataset[i], 'x', None)
        if current_graph_x is not None and current_graph_x.shape[0] > 0:
            if num_node_features == 0:  # Should have been set above
                num_node_features = current_graph_x.shape[1]
            all_train_x_list.append(current_graph_x)
        else:
    ...
    if not all_train_x_list:
        ...
        # Use num_node_features if found, else default to 4
        _num_feat = num_node_features if num_node_features > 0 else 4
        normalization_params = {'mean': np.zeros(_num_feat), 'std': np.ones(_num_feat)}
    else:
        ...
        if num_node_features == 0:
            num_node_features = all_train_x.shape[1]
    ...
    skipped_large_graphs_train = 0

    for i, graph_idx in enumerate(train_indices):
        original_graph = full_dataset[graph_idx]
        if original_graph.num_nodes > MAX_NODES_FOR_EIGS:
            skipped_large_graphs_train +=1
        
        features = extract_features_for_graph(original_graph, scaler_x)
        
        if np.isnan(features).all():
            num_nan_feature_vectors_train +=1
            # print("Warning: All features are NaN for training graph index " + str(graph_idx) + " (lh_id: " + str(original_graph.lh_id) + "). Skipping.")
            continue

        processed_train_data['X'].append(features)
        processed_train_data['y'].append(original_graph.y.numpy().flatten())
        processed_train_data['lh_ids'].append(original_graph.lh_id)
        if (i + 1) % 100 == 0:
            print("  Processed " + str(i+1) + "/" + str(len(train_indices)) + " training graphs...")
    end_time_train = time.time()
    print("Training set feature engineering completed in " + str(round(end_time_train - start_time_train, 2)) + " seconds.")
    if skipped_large_graphs_train > 0:
        print("  Skipped eigs for " + str(skipped_large_graphs_train) + " large training graphs (>" + str(MAX_NODES_FOR_EIGS) + " nodes).")
    if num_nan_feature_vectors_train > 0:
        print("Warning: Skipped " + str(num_nan_feature_vectors_train) + " training graphs due to all NaN features (after potential large graph skips).")

    start_time_test = time.time()
    num_nan_feature_vectors_test = 0
    skipped_large_graphs_test = 0
    for i, graph_idx in enumerate(test_indices):
        original_graph = full_dataset[graph_idx]
        if original_graph.num_nodes > MAX_NODES_FOR_EIGS:
            skipped_large_graphs_test +=1

        features = extract_features_for_graph(original_graph, scaler_x)

        if np.isnan(features).all():
            num_nan_feature_vectors_test +=1
            # print("Warning: All features are NaN for test graph index " + str(graph_idx) + " (lh_id: " + str(original_graph.lh_id) + "). Skipping.")
            continue

        processed_test_data['X'].append(features)
        processed_test_data['y'].append(original_graph.y.numpy().flatten())
        processed_test_data['lh_ids'].append(original_graph.lh_id)
        if (i + 1) % 50 == 0:
            print("  Processed " + str(i+1) + "/" + str(len(test_indices)) + " testing graphs...")
    end_time_test = time.time()
    print("Testing set feature engineering completed in " + str(round(end_time_test - start_time_test, 2)) + " seconds.")
    if skipped_large_graphs_test > 0:
        print("  Skipped eigs for " + str(skipped_large_graphs_test) + " large testing graphs (>" + str(MAX_NODES_FOR_EIGS) + " nodes).")
    if num_nan_feature_vectors_test > 0:
        print("Warning: Skipped " + str(num_nan_feature_vectors_test) + " test graphs due to all NaN features (after potential large graph skips).")
    ...
    if processed_train_data['X'].size > 0:
    ...
    if processed_test_data['X'].size > 0:
        ...
        print("No testing data was processed successfully.")
